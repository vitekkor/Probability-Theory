...
class chiGradient:
    ...
    def chi_gradient(self, k):
        """
        :param k: значение k Гамма-функции
        :return: значение
        """
        # return -(self.num/2)*np.log(2) - (self.num/2)*special.digamma(k/2) + (1/2)*self.sum
        return -(self.num/2)*np.log(2) + (1/2)*self.sum - (1/2)*self.num*special.digamma(k/2)
